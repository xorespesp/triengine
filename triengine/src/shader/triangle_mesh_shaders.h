...
namespace triengine::shader
{
    ...
    static const char* const kVertShadedTriangleMeshVertexShader = R"(
		////////////////////////////////////////////
		// shader inputs
		////////////////////////////////////////////
    ...
    static const char* const kVertShadedSolidTriangleMeshFragmentShader = R"(
    ...
    static const char* const kVertShadedTransparentTriangleMeshFragmentShader = R"(
        struct VertexShadingMaterial
        {
            float ambient; // ambient intensity
            float diffuse; // diffuse intensity
    ...
    static const char* const kTexShadedTriangleMeshVertexShader = R"(
		////////////////////////////////////////////
		// shader inputs
		////////////////////////////////////////////
        layout(location = 0) in vec3 vsi_vertPos; // object-space vertex position
    ...
    static const char* const kTexShadedSolidTriangleMeshFragmentShader = R"(
            ...
            in vec3 eyeDirInView)
        {
            ...
            // ambient
            const vec3 ambient = light.ambientIntensity * vec3(texture(u_material.diffuse, fsi.texCoords)) * light.color;
            
            // diffuse
            const float diff = max(dot(fragNormalInView, lightDirInView), 0.0);
            const vec3 diffuse = light.diffuseIntensity * diff * vec3(texture(u_material.diffuse, fsi.texCoords)) * light.color;
            ...
            if (light.use_blinn) {
                ...
                const vec3 halfwayDirInView = normalize(lightDirInView + eyeDirInView);
                const float spec = pow(max(dot(fragNormalInView, halfwayDirInView), 0.0), u_material.shininess);
                specular = light.specularIntensity * spec * vec3(texture(u_material.specular, fsi.texCoords)) * light.color;
            } else {
                // phong model
                const vec3 reflectDirInView = reflect(-lightDirInView, fragNormalInView);
                const float spec = pow(max(dot(eyeDirInView, reflectDirInView), 0.0), u_material.shininess);
            ...
            in vec3 eyeDirInView)
        {
            ...
            // ambient
            vec3 ambient = light.ambientIntensity * vec3(texture(u_material.diffuse, fsi.texCoords)) * light.color;

            // diffuse
            const float diff = max(dot(fragNormalInView, lightDirInView), 0.0);
            vec3 diffuse = light.diffuseIntensity * diff * vec3(texture(u_material.diffuse, fsi.texCoords)) * light.color;
        ...
        void main()
        {
            ...
            {
                resultColor = texture(u_material.diffuse, fsi.texCoords).rgb;
            }
    ...
    static const char* const kTexShadedTransparentTriangleMeshFragmentShader = R"(
        struct TextureShadingMaterial
        {
            sampler2D diffuse; // diffuse map
            sampler2D specular; // specular map
