...
namespace detail
{
    ...
    public:
        ...
        std::string_view data() const
        {
            ...
            if (!body_ptr) {
                XUTL_ERROR("Received packet with invalid data handle: {}", _data_handle);
                return std::string_view{};
            }
    ...
    public:
            ...
            size_t payload_size = 0) noexcept
        {
            ...
            try
            {
                ...
                ec = e.get_error_code();
                XUTL_ERROR("Failed to send packet (error: {})", e.what());
            }
            ...
            boost_ipc::error_code_t* ec = nullptr)
        {
            ...
            try
            {
                ...
                XUTL_ERROR("Failed to receive packet (error: {})", e.what());
            }
    ...
    const size_t payload_size)
{
    ...
    std::scoped_lock lk{ _session_lock };
    if (!_is_alive) {
        XUTL_WARN("Session is not alive, cannot send notify.");
    ...
    if (boost_ipc::no_error != _base->send_packet(
        ...
        payload_size))
    {
        XUTL_ERROR("failed to send notify");
        this->_do_close();
    }
    ...
    const std::chrono::milliseconds timeout)
{
    if (payload_size == 0 || !payload) {
        XUTL_WARN("Invalid payload for request.");
    ...
    // acquire the session lock...
    {
        ...
        if (boost_ipc::no_error != _base->send_packet(
            ...
            payload_size))
        {
            XUTL_ERROR("failed to send request");
        ...
        bool timeout_occurred{ false };
        {
                ...
                it != _state->req_map.end())
            {
                try {
                    ...
                    XUTL_TRACE("Request(seq={}) timed out, but response arrived just in time.", curr_req_id);
                }
            }
        }

        if (timeout_occurred) {
            XUTL_ERROR("Request(seq={}) timed out.", curr_req_id);
...
{
    XUTL_DEBUG("session close start..");
    const bool old_alive_flag = _is_alive.exchange(false);
    if (_recv_thread.joinable()) {
        XUTL_TRACE("terminating recv thread..");
...
void ipc_session::_do_recv()
{
    ...
    while (_is_alive)
    {
        ...
        if (std::chrono::steady_clock::now() - _state->last_peer_heartbeat.load() > kHeartBeatTimeout) {
            XUTL_ERROR("session {}: Remote disconnected (Peer heartbeat timed out)", this->get_name());
            _is_alive = false;
            break;
        }
        ...
        if (std::chrono::steady_clock::now() - last_sent_heartbeat > kHeartBeatInterval) {
            ...
            if (boost_ipc::no_error != _base->send_packet(detail::ipc_packet_type::heartbeat, 0)) {
                XUTL_ERROR("failed to send heartbeat");
            }
            ...
            if (_cb_req_pck)
            {
                ...
                if (!rep_pck_buff.empty()) {
                    if (boost_ipc::no_error != _base->send_packet(
                        ...
                        rep_pck_buff.size()))
                    {
                        XUTL_ERROR("failed to send response");
                    }
                } else {
                    XUTL_WARN("response packet empty");
                }
            } 
            else 
            {
                XUTL_WARN("request handler not registered");
            }
        }
        else if (recvd_pck.type() == detail::ipc_packet_type::response)
        {
            ...
            if (!req_node.empty()) {
                ...
                try {
                    ...
                    XUTL_WARN("Failed to set value for request ID {}: {}", recvd_pck.id(), e.what());
                }
            } else {
                XUTL_WARN("Received response for unknown request ID: {}", recvd_pck.id());
            }
        }
        else
        {
            XUTL_WARN("Got unknown packet ({})", static_cast<int>(recvd_pck.type()));
        }
    ...
    XUTL_TRACE("IPC session recv terminated.");
}
    ...
    while (_is_listening)
    {
...

        {
            ...
            if (_ctx->sessions.size() >= _ctx->max_sessions) {
                XUTL_WARN("New session connection denied (maximum number of sessions reached)");
                continue;
            }
            ...
            new_session = std::make_shared<ipc_session>(
                ...
                [weak_self = std::weak_ptr{ shared_from_this() }](std::shared_ptr<ipc_session> session)
                {
                    ...
                    XUTL_INFO("Session {} closed. Removing from server.", session_name);

                    auto self = weak_self.lock();
                    if (!self) {
                        XUTL_WARN("Server instance no longer exists, cannot remove session.");
                        return;
                    }
        ...
        XUTL_INFO("New session accepted! (name: {})", new_session_name);
        ...
        if (_on_session_connect) {
            ...
            XUTL_WARN("No session connect callback set, cannot notify about new session.");
        }
    ...
    const std::chrono::milliseconds timeout)
{
    std::scoped_lock ctx_lk{ _ctx_lock };
    if (_ctx) {
        XUTL_ERROR("Already connected or connection in progress.");
        return false;
    }
    ...
    try {
        ...
        XUTL_ERROR("Failed to open server handshake resources: {}", e.what());
        return false;
    }
    ...
    if (!mq_handshake_s2c->timed_receive(
        ...
        boost::posix_time::second_clock::universal_time() + boost::posix_time::milliseconds(timeout.count())))
    {
        XUTL_ERROR("Handshake with server '{}' timed out.", server_name);
        return false;
    }
    ...
    auto new_session = std::make_shared<ipc_session>(
        ...
        {
            XUTL_INFO("Client disconnected!");

            auto self = weak_self.lock();
            if (!self) {
                XUTL_WARN("Client instance no longer exists, cannot handle disconnection.");
                return;
            }
    ...
    XUTL_INFO("Connected to server! (session name: {})", new_session_name);
    return true;
}
