...
uniform mat4 u_cameraProj; // camera's projection matrix
uniform vec3 u_textViewSpacePos; // view(camera) space position of the 3d label (for depth comparison)
...
void main()
{
    ...
    vo.viewSpacePos = u_textViewSpacePos;
    
    // Transform view space to screen space
    vec4 clipSpacePos = u_cameraProj * vec4(vo.viewSpacePos, 1.0); // view space -> clip space
    vec3 ndcSpacePos = clipSpacePos.xyz / clipSpacePos.w; // clip space -> NDC space (perspective division)
    vo.screenSpacePos = ndcSpacePos.xy * 0.5 + 0.5; // NDC space [-1, 1] -> screen space [0, 1]
}